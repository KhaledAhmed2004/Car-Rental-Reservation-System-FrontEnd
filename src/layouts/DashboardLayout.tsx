...
//       <div className="flex gap-4 pt-[96px] mx-auto max-w-7xl">
//         {/* Sidebar */}
//         <div className="h-[calc(100vh-96px)] w-[20%] bg-white rounded-lg p-6 space-y-1 hidden">
//           {/* user-specific links */}
//           {userRole === "user" && (
//             <>
//               <DashboardNavLink
...
const DashboardLayout = () => {
  ...
  function renderUserLinks() {
    return (
      <>
        <DashboardNavLink icon={<LuLayoutDashboard />} to="userOverview" label="Overview" />
        <DashboardNavLink icon={<LuHistory />} to="myBookings" label="My Bookings" />
  ...
  function renderAdminLinks() {
    return (
      <>
        <DashboardNavLink icon={<LuLayoutDashboard />} to="adminOverview" label="Overview" />
        <DashboardNavLink icon={<LiaCarSideSolid />} to="manageCar" label="Manage Cars" />
        <DashboardNavLink icon={<LuClipboardList />} to="manageBookings" label="Manage Bookings" />
        <DashboardNavLink icon={<MdKeyboardReturn />} to="manageReturnCar" label="Manage Return Cars" />
        <DashboardNavLink icon={<HiOutlineUsers />} to="manageUser" label="User Management" />
      </>
    );
  }
};
  ...
  return (
    <NavLink to={to}>
      {({ isActive }) => (
        <div
          className={`group flex items-center gap-2 font-medium p-2 rounded-lg transition-all ${
            isActive ? "bg-slate-100 text-blue-600" : "hover:bg-slate-100 hover:text-blue-600"
          }`}
        >
          <span className="mr-2 transition-transform group-hover:translate-x-2">
            {icon}
